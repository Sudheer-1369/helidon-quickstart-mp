...
public class CollectorsMethodsPractice {
  ...
  public static void main(String[] args) {
    ...
    List<Employee> employees =
        employeeList.stream().filter(s -> s.age > 20).collect(Collectors.toList());
    System.out.println(employees);

    // Setting
    Set<Employee> employeeSet =
        employeeList.stream().filter(s -> s.age > 20).collect(Collectors.toSet());
    ...
    Map<Boolean, List<Employee>> bySalaryLevel =
        employeeList.stream().collect(Collectors.partitioningBy(s -> s.getSalary() >= 25));
