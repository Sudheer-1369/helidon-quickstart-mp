...
public class StreamMap {
  ...
  static final Map<String, List<String>> filteredMap =
      Arrays.stream(new String[] {"1", "2", "3"})
          .collect(
              Collectors.groupingBy(
                  s -> "key", Collectors.filtering(s -> s.equals("1"), Collectors.toList())));

  public static void main(String[] args) {
    System.out.println("Map obtained from the stream is " + map);

    System.out.println(
        "Map obtained by collecting and grouping the values from stream is " + multiValuedMap);

    System.out.println(
        "Map obtained by collecting and filtering the values from stream is " + filteredMap);
  }
}
