...
public class GenericMethods {
  ...
  public List<String> fromArrayToList(int[] a, Integer[] b) {
    ...
    // The type argument of a Stream class doesnt support a primitive type but when we pass an
    // array/list of primitive types
    // then it will produce something called primitive Streams like IntStream, DoubleStream,
    // LongStream. Refer below cases
    IntStream stream = Arrays.stream(a);
    Stream<Integer> stream1 = Arrays.stream(b);

    Stream<Integer> stream2 =
        stream.boxed(); // this will wrap each element of instream with Integer

    // In case of IntStream the map method takes an IntUnaryOperator as the input only which maps
    // and int to int.
  ...
  public <M, S, K extends Number> List<K> fromArrayToListGen(M[] a, S[] b) {
    ...
    //        return stream.map(s->1).collect(Collectors.toList()); // This will throw an error
    // stating incompatible types found : found - integer, req : K

    // When the above return statment is used the compiler will complain that the map is always
    // returning the integer so make the type K as lower bound of integer.
    return null;
  }
  ...
  // Way-2 :
  // This way is better than way-1 since here we pass the method that needs to be applied in the map
  // is also made generic using
