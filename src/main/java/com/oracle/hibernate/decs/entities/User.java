...
@NamedNativeQueries({
  @NamedNativeQuery(
      name = "findAllUsers",
      query = "SELECT FULLNAME,EMAIL,PASSWORD FROM usersdb.USERS",
      resultSetMapping = "UserResult"),
  @NamedNativeQuery(name = "deleteUserById", query = "DELETE FROM usersdb.USERS WHERE ID= :ID"),
  @NamedNativeQuery(
      name = "deleteUserByCode",
      query = "DELETE FROM usersdb.USERS WHERE CODE= :CODE"),
  @NamedNativeQuery(
      name = "getUserById",
      query = "SELECT FULLNAME,EMAIL,PASSWORD FROM usersdb.USERS WHERE CODE= :CODE")
})
@SqlResultSetMapping(
    name = "UserResult",
    classes = {
