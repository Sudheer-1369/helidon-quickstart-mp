...
* The ResultSet interface declares getter methods for retrieving column values from the current row. The values can be retrieved using either the index number of the column or the alias or name of the column.
...
* Note that stmt uses the method executeBatch to send the batch of insertions, not the method executeUpdate, which sends only one command and returns a single update count. The DBMS executes the commands in the order in which they were added to the list of commands
